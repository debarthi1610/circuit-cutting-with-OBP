...
def perform_cutting(circuit: QuantumCircuit, observable: SparsePauliOp, backend, op_budget: OperatorBudget) -> int:
    ...
    num_subexps_cut = cutting(synth_circuit, observable, backend)
    ...
    if type(bp_circuit) != QuantumCircuit:
        return num_subexps_cut, None
    else:
        num_subexps_obp_cut = cutting(bp_circuit, bp_obs, backend)
        print(num_subexps_obp_cut)
        return num_subexps_cut, num_subexps_obp_cut
